...
### 2.4. 时间预测问题的形式化定义 
#### 2.4.1. 核心构成：观测与预测
智能体在时间步 $t$ 接收一个观测向量 $\phi_t \in \mathbb{R}^n$，并据此做出一个**标量预测** $v_t$。
#### 2.4.2. 预测目标：基于累积奖励的未来值
预测的评估依据是未来某个标量（称为**累积量，cumulant**）的总和。该累积量可以是观测向量 $\phi_t$ 的任意标量分量，**最常见的选择是奖励信号 $r_t$**。
...
$$
其中：
- $r_t$ 是观测 $\phi_t$ 的一个分量，是 $t$ 时刻的累计量； 
- $T$：智能体的生命周期（总时间步数）；
...
每个智能体的策略由一个权重向量 $\mathbf{w} \in \mathbb{R}^n$ 参数化，其预测通过**线性组合特征向量与权重**实现：
$$
v_t = \sum_{i=1}^{n} w_{t-1}[i] \cdot \phi_t[i],
\quad \text{其中 } \phi_t[i] \text{ 是观测向量 } \phi_t \text{ 的第 } i \text{ 个分量}。
$$
权重向量的索引为 $t-1$，表示预测 $v_t$ 是在**使用当前观测 $\phi_t$ 更新权重之前**做出的； 
...
#### 4.1.2. 参数化方式
- 引入辅助向量 $\beta \in \mathbb{R}^n$ 用于参数化步长；
- 权重更新中的实际步长设为：$\alpha[i] = e^{\beta[i]}$ 
> 📌 **优势**：指数形式确保步长始终为正，且便于梯度优化。

#### 4.1.3. 步长参数的更新规则
通过最小化预测误差对步长参数进行元学习（meta-learning），来得到使得预测误差最小的每个步长参数：
$$
\beta_t[i] = \beta_{t-1}[i] - \frac{\theta}{e^{\beta[i]}} \cdot \frac{\partial (v_t - G^\lambda_t)^2}{\partial \beta[i]},
$$
其中：
- $\frac{\theta}{e^{\beta[i]}}$ 为元步长参数（meta-step-size parameter）；  
- $G^\lambda_t$ 为 $\lambda$-回报目标；
- 求导部分表示预测误差对 $\beta[i]$ 的敏感度。

:::note
**情况1：当主步长 $α[i]$ 已经很大时**
- $α[i] = e^{\beta[i]}$ 很大，意味着 $β[i]$ 的值也比较大。
- 根据公式，此时元步长 $η$ 会变得很小。
- 为什么这么做？ 因为 $α[i]$ 是通过指数函数 $e^x$ 生成的，当 $x$ 很大时，$e^x$ 的值对 $x$ 的微小变化非常敏感。换句话说，当 $α[i]$ 已经很大时，对 $β[i]$ 做一点点改动，都会导致 $α[i]$ 发生剧烈的变化。为了防止更新过猛导致系统不稳定，我们需要非常谨慎、保守地去调整 $β[i]$。因此，一个很小的元步长 $η$ 是必需的，它起到了稳定器的作用。

**情况2：当主步长 $α[i]$ 非常小时**
- $α[i] = e^{\beta[i]}$ 很小，意味着 $β[i]$ 是一个绝对值很大的负数。
- 根据公式，此时元步长 $η$ 会变得很大。
- 为什么这么做？ 当 $α[i]$ 已经很小时，说明这个特征的学习几乎停滞了。同样因为指数函数的特性，此时即使 $β[i]$ 变化一点，$α[i]$ 的绝对变化量也非常微小。如果元步长 $η$ 也很小，那么 $β[i]$ 的更新就会非常缓慢，可能需要很久才能让 $α[i]$ “逃离”这个极小的区域。为了加速对这种“僵死”步长的调整，系统采用了一个更大的元步长 $η$，使得 $β[i]$ 可以被更快地调整到一个合理的范围。

**说人话**：

因为元梯度与 $\beta[i]$ 成比例，为了抵消掉这个我们不想要的缩放效应。 
那么我们就在它的元学习率 $\theta$的基础上除以一个 $e^{\beta[i]}$ ，把这个效应给抵消掉，使得元学习过程更加稳定高效。
::: 
...
- **初始步长 $\alpha^{init}$**：所有特征的初始步长，最好不超过 $10^{-4}$。
...
对于**线性回归**且每个维度使用独立步长 $\alpha_t[i]$，可简化为  
$$
...
#### 4.3.1. 🛠 实现机制
步长衰减在实现上十分简单。设 $\alpha_t$ 为时间 $t$ 的步长参数。
...
- 在评估环境下对 SwiftTD 的初始步长 $\alpha_{\text{init}}$ 与元步长 $\theta$ 进行超参敏感性分析。  
...
#### 🔍 结果观察：
- **SwiftTD** 在所有 $\alpha_{\text{init}}$ 与 $\theta$ 组合下均表现**稳定且性能良好**。  
